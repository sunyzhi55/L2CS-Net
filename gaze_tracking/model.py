...
class EyeModel:
    ...
    def get_gaze(self, frame, imshow=False):
        ...
        if frame is None or frame.size == 0:
            print("Empty frame passed to get_gaze(), skipping")
            return None
    ...
    def get_FaceFeatures(self, frame, imshow=False):
        ...
        if len(face_boxes) < 1:
            ...
            cv2.imwrite(os.path.join(self.dir, "results", "no_face.jpg"), frame)
            # Return a default shaped array to keep caller code stable.
            # Callers expect a (3, N) array (homogeneous coordinates) where N is number
            # of landmark points (facial_landmark_35 has 35 points). Return zeros.
            try:
                n_landmarks = 35
                return np.zeros((3, n_landmarks))
            except Exception:
