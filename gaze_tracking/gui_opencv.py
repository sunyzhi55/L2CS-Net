...
# Headless flag: when True, skip all GUI operations (namedWindow, imshow, setWindowProperty)
HEADLESS = False


def set_headless(flag: bool):
    """Set headless mode for GUI functions.

    When headless is True, display/window related functions will return
    immediately without creating windows. Use this when running on servers
    without a display (no X11) or when you cannot install xvfb.
    """
    global HEADLESS
    HEADLESS = bool(flag)
...
def get_out_video(cap, output_path, file_name = "calibrate.mp4", scalewidth = 1):
    ...
    fps = 20
    # Use 'mp4v' for wider compatibility when system ffmpeg/x264 is not available.
    # If you have ffmpeg with x264 compiled, you may change to 'avc1' or 'H264'.
    out_video = cv2.VideoWriter( os.path.join(output_path,file_name), cv2.VideoWriter_fourcc(*'mp4v'), fps, (scalewidth*width, height))
...
def getScreenSize():
    ...
    #     # Likely running on a headless server without a display (e.g., no X11).
    #     # Provide a sensible default and warn the user. If you need a real display,
    #     # run under Xvfb or set DISPLAY appropriately.
    #     print(f"Warning: could not get screen size ({e}). Falling back to default 1920x1080.")
    #     width, height = 1920, 1080
    #     # approximate mm values for a typical 24-inch monitor
    #     width_mm, height_mm = 344, 193
        # return width, height, width_mm, height_mm
    return 1920, 1080, 344, 193
